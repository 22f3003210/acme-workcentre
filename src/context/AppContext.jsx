      ...
      supabase.from("projects").select("*").then(({ data, error }) => {
        if (!error && data) {
          const mapped = data.map(p => ({
            ...
            status: p.status,
            stage: p.stage || p.business_details?.stage || "On-Going Stage",
            auditSubStage: p.audit_sub_stage || p.business_details?.auditSubStage || "pre_audit_virtual",
            discontinuedFromStage: p.discontinued_from_stage || p.business_details?.discontinuedFromStage || null,
            discontinuedReason: p.discontinued_reason || p.business_details?.discontinuedReason || null,
            discontinuedDate: p.discontinued_date || p.business_details?.discontinuedDate || null,
            stageHistory: p.stage_history || p.business_details?.stageHistory || [],
            preAuditData: p.pre_audit_data || p.business_details?.preAuditData || {},
            projectPlan: p.project_plan || p.business_details?.projectPlan || {},
            metaLeadData: p.meta_lead_data || p.business_details?.metaLeadData || {},
